...
const FloatingGenAI = () => {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      text: `Hola. Soy tu asistente ${BRAND.shortName} de soporte operativo. Estoy aqui para ayudarte paso a paso con permisos, rutas, errores y flujos de cada modulo.`,
      fuentes: []
    }
  ]);
  ...
  useEffect(() => {
    ...
    setMessages([
      {
        role: 'assistant',
        text: `Hola ${user.name}. Soy ${BRAND.aiSupportLabel}. Preguntame cualquier cosa del ecosistema y te guio con una respuesta clara y accionable.`,
        fuentes: []
      }
    ]);
  }, [user?.name]);
    ...
    <>
        ...
        <div className={`fixed z-[80] bottom-24 right-5 ${expanded ? 'w-[min(92vw,560px)]' : 'w-[min(92vw,420px)]'} bg-white border border-slate-200 rounded-3xl shadow-2xl shadow-slate-900/20 overflow-hidden`}>
          <div className="px-4 py-3 bg-gradient-to-r from-slate-900 via-indigo-900 to-indigo-600 text-white flex items-center gap-2">
            <div className="p-1.5 rounded-xl bg-white/10 border border-white/20">
              <img src={BRAND.logoPath} alt={BRAND.shortName} className="w-7 h-7 object-cover rounded-lg" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-[11px] font-black uppercase tracking-widest truncate">{BRAND.aiSupportLabel}</p>
      ...
      >
        ...
        {open ? (
          ...
          <img src={BRAND.logoPath} alt={BRAND.aiAssistantLabel} className="relative z-10 w-11 h-11 object-cover rounded-xl border border-white/30 shadow-lg" />
        )}
      </button>
    </>
  );
