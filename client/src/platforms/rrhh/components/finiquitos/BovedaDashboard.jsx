...
import { Users, AlertCircle, Building2, FileText, Search, Loader2, Landmark, Briefcase, MapPin, Bell } from 'lucide-react';
...
export default function BovedaDashboard({ bovedaEmployees, bovedaLoading, projects, onOpenFiniquito }) {
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-6">
                {[
                    { label: 'Total Bajas', value: bovedaEmployees.length, icon: Users, color: 'from-violet-500 to-violet-700', sub: 'histórico' },
                    { label: 'Finiquitados', value: bovedaEmployees.filter(e => e.status?.toUpperCase() === 'FINIQUITADO' || e.status?.toUpperCase() === 'DE BAJA').length, icon: FileText, color: 'from-slate-500 to-slate-700', sub: 'desvinculados' },
                    { label: 'Rechazados', value: bovedaEmployees.filter(e => e.status?.toUpperCase() === 'RECHAZADO').length, icon: AlertCircle, color: 'from-rose-500 to-rose-700', sub: 'proceso fallido' },
                    { label: 'Retirados', value: bovedaEmployees.filter(e => e.status?.toUpperCase() === 'RETIRADO').length, icon: Building2, color: 'from-amber-500 to-amber-600', sub: 'abandono' },
                ].map((s, i) => (
            ...
            <div className="filter-bar bg-white border border-slate-100 rounded-xl sm:rounded-2xl p-3 sm:p-4 mb-5 flex flex-wrap gap-2 sm:gap-3 shadow-sm">
                <div className="relative flex-1 min-w-full sm:min-w-[200px]">
                    ...
                    <input type="text" placeholder="Buscar..." value={bovedaSearchTerm}
                        onChange={e => setBovedaSearchTerm(e.target.value)}
                ...
                <div className="flex-1 sm:flex-none w-full sm:w-[200px] z-50">
                    <SearchableSelect
                        value={bovedaFilterCeco}
                        onChange={val => setBovedaFilterCeco(val)}
                        placeholder="CECOs"
                        options={[
                            { value: '', label: 'CECOs' },
                            ...bovedaCecos.map(c => ({ value: c, label: c }))
                        ]}
                    />
                </div>
                <div className="flex-1 sm:flex-none w-full sm:w-[200px] z-50">
                    <SearchableSelect
                        value={bovedaFilterProj}
                        onChange={val => setBovedaFilterProj(val)}
                ...
                <div className="flex items-center justify-center gap-2 bg-slate-100 rounded-xl px-3 py-2.5 text-[10px] font-black text-slate-500">
                    <Users size={12} /> {bovedaFiltered.length} mostrando
                </div>
            </div>
            ...
            <div className="bg-white rounded-xl sm:rounded-[2rem] border border-slate-100 p-4 sm:p-6 shadow-sm">
                {bovedaLoading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                        {bovedaFiltered.map(emp => (
                            <div key={emp._id} 
                                onClick={() => onOpenFiniquito(emp)}
                                className={`p-5 bg-white border-2 rounded-2xl transition-all group hover:shadow-md relative cursor-pointer \${emp.alerts === 2 ? 'border-red-200 hover:border-red-400' :
