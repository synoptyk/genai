...
import { candidatosApi, proyectosApi, descuentosApi } from '../rrhhApi';
...
const Descuentos = () => {
    ...
    const handleBulkUpload = async (data) => {
        try {
            const res = await descuentosApi.uploadExcel(period, data);
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            setTiposDescuento(tiposRes.data || []);
            ...
            (candRes.data || []).forEach(emp => {
                ...
                initialEdits[emp._id] = empTx.map(tx => ({
                    ...
                    tipoDescuentoRef: tx.tipoDescuentoRef._id || tx.tipoDescuentoRef,
                    monto: tx.monto || 0,
            ...
            console.error('Error fetching descuentos data:', e);
            showAlert('Error al cargar datos legales de descuentos', 'error');
        } finally {
            setLoading(false);
        }
    }, [period]);
    ...
    const filtered = useMemo(() => {
        return empleados.filter(e => {
            ...
            if (['De Baja', 'Retirado', 'Rechazado'].includes(e.status)) return false;
            
            if (e.status === 'Finiquitado') {
                const finiquitoMes = e.fechaFiniquito ? new Date(e.fechaFiniquito).toISOString().slice(0, 7) : null;
                if (finiquitoMes !== period) {
                    return false;
                }
            }
    ...
    const getTotalDescuentos = (empId) => {
            ...
            return {
                ...
                [emp._id]: list.map(item => {
                    if (item.id === itemId) {
                        ...
                        // Auto-calc triggers
                        const tipo = tiposDescuento.find(t => t._id === item.tipoDescuentoRef);
    ...
    const handleSave = async (empId) => {
        ...
        try {
            ...
            await descuentosApi.saveTransacciones(empId, period, { transacciones });
            showAlert('Descuentos sincronizados correctamente (DT/LRE)');
            setExpandedId(null);
            fetchData();
        } catch (e) {
            showAlert('Error al procesar los descuentos', 'error');
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleExport = () => {
        ...
        filtered.forEach(e => {
            ...
            if (list.length === 0) {
                rows.push({
                    ...
                    'SUELDO BASE': e.sueldoBase || 0, 'CÓDIGO DT': '', 'TIPO DESCUENTO': 'Sin Descuentos', 'MONTO': 0
                });
            } else {
                list.forEach(tx => {
                    const tipo = tiposDescuento.find(t => t._id === tx.tipoDescuentoRef);
                    rows.push({
                        'RUT': e.rut, 'NOMBRE': e.fullName, 'CARGO': e.position, 'CECO': proj?.centroCosto || e.ceco || '', 'PERÍODO': period,
        ...
        XLSX.utils.book_append_sheet(wb, ws, `LRE_Descuentos_${period}`);
        XLSX.writeFile(wb, `Descuentos_LRE_${period}.xlsx`);
    };
    ...
    return (
        <div className="min-h-full bg-slate-50/50 p-6 pb-24">
            ...
            {/* HEADER METRICS AND FILTERS REMAIN SAME */}
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase flex items-center gap-2">
                            ...
                            <span className="px-2 py-1 bg-rose-100 text-rose-700 rounded-lg text-[10px] tracking-widest font-black border border-rose-200">LRE COMPLIANT</span>
                        </h1>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1">
                            Gestión legal de retenciones · {filtered.length} colaboradores activos
                        </p>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    <input type="month" value={period} onChange={e => setPeriod(e.target.value)} className="px-4 py-3 bg-white border border-slate-200 rounded-2xl text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-rose-200" />
            ...
            {activeTab === 'asignacion' ? (
                ...
                {[
                    { label: 'Total Retenciones Período', value: fmt(totalGeneral), icon: DollarSign, color: 'rose' },
                    { label: 'Colaboradores con Descuento', value: conDescuentos, icon: ShieldAlert, color: 'amber' },
            ...
            <div className="bg-white/80 backdrop-blur-xl border border-white rounded-[2.5rem] p-6 mb-6 flex flex-wrap items-center gap-4 shadow-xl">
                <div className="flex-1 min-w-[250px] relative group">
                    ...
                    <input type="text" placeholder="Buscar por nombre o RUT..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-bold text-slate-600 focus:outline-none focus:ring-2 focus:ring-rose-100" />
                </div>
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                                            ...
                                            <div className="hidden md:block">
                                                ...
                                                {emp.status === 'Finiquitado' ? (
                                                    <p className="text-[10px] font-black text-rose-600 flex items-center gap-1">
                                                        <XCircle size={12} className="text-rose-500" /> FINIQUITADO ESTE MES
                                                    </p>
                                                ) : (
                                            ...
                                            {total > 0 ? (
                                                <div className="flex items-center gap-2">
                                                    <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden hidden sm:block">
                                                        <div className={`h-full ${superaMaxLegal ? 'bg-rose-600' : 'bg-rose-400'}`} style={{ width: `${Math.min(percBase, 100)}%` }} />
                                                    </div>
                                                    <span className={`px-4 py-1.5 ${superaMaxLegal ? 'bg-rose-100 text-rose-700 border-rose-200' : 'bg-slate-100 text-slate-700 border-slate-200'} border rounded-xl text-[10px] font-black`}>
                                                        {fmt(total)}
                                                    </span>
                                                </div>
                                            ) : (
                                                <span className="px-4 py-1.5 bg-slate-50 text-slate-400 border border-slate-100 rounded-xl text-[10px] font-black">Sin Descuentos</span>
                                            )}
                                            {superaMaxLegal && !isOpen && (
                                                <Tooltip content="Alerta LRE: Supera el límite de retención recomendado (45% s/ Base).">
                                        ...
                                        <div className="px-8 pb-8 bg-slate-50 border-t border-slate-100 relative">
                                            ...
                                            <div className="flex flex-col gap-6">
                                                {items.length === 0 && (
                                                    <div className="text-center py-6 text-slate-400 font-bold text-sm bg-white border border-slate-100 border-dashed rounded-2xl">
                                                        No hay descuentos registrados en este periodo. Haz clic en el botón de abajo para agregar uno.
                                                    </div>
                                                )}
                                                    ...
                                                    return (
                                                        <div key={item.id} className="relative bg-white p-5 rounded-3xl border border-slate-200 shadow-sm animate-in slide-in-from-top-2">
                                                            <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100">
                                                                <div className="flex items-center gap-3">
                                                                    <div className="w-8 h-8 rounded-full bg-rose-50 text-rose-600 flex items-center justify-center text-xs font-black">
                                                                        {index + 1}
                                                                    </div>
                                                                ...
                                                                <div className="flex items-center gap-3">
                                                                    {limite > 0 && (
                                                                        <span className={`text-[9px] font-black px-2.5 py-1 rounded-lg uppercase tracking-widest ${excedido ? 'bg-rose-100 text-rose-700' : 'bg-slate-100 text-slate-500'}`}>
                                                                            Tope {limite}%
                                                                        </span>
                                                                    )}
                                                                    <button onClick={() => removeRetencion(emp._id, item.id)} className="p-1.5 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors">
                                                                        <Trash2 size={16} />
                                                                    </button>
                                                                </div>
                                                            </div>
                                                                    ...
                                                                    {isInasistencia && (
                                                                        <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200">
                                                                            <div className="flex items-center justify-between mb-2">
                                                                                ...
                                                                                <button onClick={() => handleAddFechaInasistencia(emp, item.id)} className="text-[9px] font-black text-rose-600 bg-rose-50 px-2 py-1 rounded hover:bg-rose-100 transition-colors">
                                                                                    + Añadir Día
                                                                                </button>
                                                                            </div>
                                                                            <div className="flex flex-col gap-2">
                                                                                {item.fechasInasistencia?.map((fecha, idx) => (
                                                                                    <div key={idx} className="flex items-center gap-2">
                                                                                        <input type="date" value={fecha} onChange={e => handleUpdateFechaInasistencia(emp, item.id, idx, e.target.value)} className="w-full text-xs font-bold text-slate-700 bg-white border border-slate-200 py-1.5 px-3 rounded-lg focus:outline-none focus:ring-1 focus:ring-rose-200" />
                                                                                        <button onClick={() => handleRemoveFechaInasistencia(emp, item.id, idx)} className="text-slate-400 hover:text-rose-500"><Trash2 size={14} /></button>
                                                                                    </div>
                                                                                ))}
                                                                            </div>
                                                                        </div>
                                                                    ...
                                                                    {isAtraso && (
                                                                        <div className="flex gap-2">
                                                                            <div className="flex-1">
                                                                                ...
                                                                                <input type="date" value={item.fechaAtraso || ''} onChange={e => updateItem(emp, item.id, 'fechaAtraso', e.target.value)} className="w-full py-2 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-rose-100" />
                                                                            </div>
                                                                            <div className="w-24">
                                                                                <span className="block text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 ml-1">Horas</span>
                                                                                <input type="number" step="0.5" min="0" value={item.cantidad || ''} onChange={e => updateItem(emp, item.id, 'cantidad', e.target.value)} className="w-full py-2 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-rose-100 text-center" placeholder="0" />
                                                                            </div>
                                                                        </div>
                                                                    )}
                                                                    ...
                                                                    {!isInasistencia && !isAtraso && (
                                                                        <div className="flex flex-col gap-3">
                                                                            <div>
                                                                                ...
                                                                                <select value={item.modalidad} onChange={e => updateItem(emp, item.id, 'modalidad', e.target.value)} className="w-full py-2 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-rose-100">
                                                                                    <option value="Totalidad">Descuento en su Totalidad</option>
                                                                                    <option value="Cuotas">Descuento en Cuotas</option>
                                                                                </select>
                                                                            </div>
                                                                ...
                                                                <div className="lg:col-span-7 flex flex-col gap-3">
                                                                    <div className="flex gap-3">
                                                                        <div className="w-1/3 relative">
                                                                            <span className="block text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 ml-1">Monto a Descontar</span>
                                                                            <div className="relative">
                                                                                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-bold pointer-events-none">$</span>
                                                                                <input type="number" min="0" value={item.monto || ''} onChange={e => updateItem(emp, item.id, 'monto', e.target.value)} className={`w-full py-2.5 pl-8 pr-3 bg-slate-50 border ${excedido ? 'border-rose-300 bg-rose-50/50' : 'border-slate-200'} rounded-xl text-sm font-black text-slate-700 focus:outline-none focus:ring-2 focus:ring-rose-100 transition-all`} placeholder="0" />
                                                                            </div>
                                                                        </div>
                                                                        <div className="w-2/3">
                                                                            <span className="block text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 ml-1">Describa el Motivo del Descuento</span>
                                                                            <input type="text" value={item.nota || ''} onChange={e => updateItem(emp, item.id, 'nota', e.target.value)} className="w-full py-2.5 px-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-rose-100" placeholder="Escriba aquí los detalles..." />
                                                                        </div>
                                                                    </div>
                                                                    <div>
                                                                        <span className="block text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 ml-1 flex items-center gap-1"><LinkIcon size={10} /> Enlace de Respaldo (Opcional)</span>
                                                                        <input type="url" value={item.respaldoUrl || ''} onChange={e => updateItem(emp, item.id, 'respaldoUrl', e.target.value)} className="w-full py-2 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-500 focus:outline-none focus:ring-2 focus:ring-rose-100 placeholder:font-normal" placeholder="https://drive.google.com/... (Link a comprobante, contrato o correo)" />
                                                                    </div>
                                                                </div>
                                                ...
                                                <div className="relative mt-2">
                                                    <button onClick={() => setShowAddMenu(showAddMenu === emp._id ? null : emp._id)} className="w-full py-4 border-2 border-dashed border-slate-200 hover:border-rose-300 bg-white hover:bg-rose-50 rounded-2xl flex items-center justify-center gap-2 text-[10px] font-black text-slate-500 hover:text-rose-600 uppercase tracking-widest transition-all">
                                                        <Plus size={16} /> Agregar Retención / Descuento
                                                    </button>
                                            ...
                                            <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-200 border-dashed">
                                                <div className="flex flex-col">
                                                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Total Retenciones (Serie 4000)</span>
                                                    <div className="flex items-center gap-3">
                                                        <span className="text-xl font-black text-rose-600">{fmt(total)}</span>
                                                        {superaMaxLegal && <span className="px-2 py-1 bg-rose-100 text-rose-700 text-[9px] font-black rounded-lg uppercase tracking-widest border border-rose-200 flex items-center gap-1"><ShieldAlert size={10} /> Alerta DT (&gt;45%)</span>}
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button onClick={() => handleSave(emp._id)} disabled={saving} className="px-6 py-2.5 bg-rose-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-700 transition-all disabled:opacity-50 flex items-center gap-2 shadow-lg shadow-rose-200">
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="border-b-2 border-slate-100">
                                            ...
                                            <th className="py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Tipo Descuento</th>
                                    ...
                                    <tbody className="divide-y divide-slate-50">
                                        {transaccionesPeriodo.map(tx => {
                                            ...
                                            const tipo = tx.tipoDescuentoRef; // Already populated from backend
                                            return (
                                                <tr key={tx._id} className="hover:bg-slate-50/50 transition-colors">
                                                    ...
                                                    <td className="py-4 pr-4">
                                                        ...
                                                        {tx.estadoAprobacion === 'Rechazado' && <span className="flex w-max items-center gap-1.5 px-3 py-1 bg-rose-100 text-rose-700 rounded-xl text-[10px] font-black uppercase tracking-widest"><XCircle size={12}/> Rechazado</span>}
                                                    </td>
                                                    <td className="py-4">
                                                        {tx.estadoAprobacion === 'Aprobado' && tx.respaldoUrl ? (
                                                            ...
                                                            <div className="text-[10px] text-rose-600 bg-rose-50 p-2 rounded-xl italic font-medium max-w-xs">
            ...
            <BulkUploadModal 
                ...
                title="Carga Masiva de Descuentos"
                templateHeaders={['RUT', 'CÓDIGO DT', 'MONTO', 'CANTIDAD', 'JUSTIFICACIÓN']}
            />
        </div>
    );
};
