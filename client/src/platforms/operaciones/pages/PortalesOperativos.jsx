...
const PortalesOperativos = () => {
    ...
    // New states implied by the diff for viewHistory
    ...
    const fetchData = async () => {
        ...
        try {
            const [resUsers, resStats] = await Promise.all([
                ...
                api.get(`/ api / auth / stats / portales`)
            ]);
            setUsers(resUsers.data || []); // Changed from setUsuarios to setUsers to match existing state
            setStats(resStats.data || {});
            setError(null); // Moved from original try block
        } catch (error) { // Changed err to error
            console.error("Error fetching portal management data:", error); // Changed err to error
    ...
    const toggleUserStatus = async (user) => { // Renamed from handleToggleStatus
        ...
        try {
            await api.put(`/ api / auth / users / ${ user._id } `, { status: newStatus }); // Changed axios to api, removed authHeader
            fetchData();
        } catch (error) { // Changed err to error
    ...
    const deleteUser = async (user) => { // Renamed from handleDeleteUser
        ...
        try {
            await api.delete(`/ api / auth / users / ${ user._id } `); // Changed axios to api, removed authHeader
            fetchData();
        } catch (error) { // Changed err to error
    ...
    const viewHistory = async (user) => {
        // Original line: setHistoryModal({ open: true, user, logs: [] });
        // Original line: setLoadingHistory(true);
        try {
            const res = await api.get(`/ api / auth / users / ${ user._id } /history`); / / Changed axios to api, removed authHeader
setHistory(res.data || []); // New state update
setSelectedUser(user); // New state update
setShowHistory(true); // New state update
            // Original line: setHistoryModal(prev => ({ ...prev, logs: res.data })); - removed as per diff
        } catch (error) { // Changed err to error
    console.error("Error loading user history:", error); // Changed err to error
} finally {
