...
const FinancialDiaTable = ({ 
  ...
  const metaPts = metaConfig.metaProduccionDia || metaConfig.metaDiaria || 7.5;
  ...
  const getSortedTechs = (mode) => {
    return [...tecnicos]
      ...
      .sort((a, b) => {
        if (mode === 'clp') {
          const baremoA = getBaremo(a.proyecto || a.cliente);
          const baremoB = getBaremo(b.proyecto || b.cliente);
          const totalA = Object.values(a.dailyMap || {}).reduce((acc, d) => acc + (typeof d === 'object' ? (d.pts || 0) : d), 0) * baremoA;
          const totalB = Object.values(b.dailyMap || {}).reduce((acc, d) => acc + (typeof d === 'object' ? (d.pts || 0) : d), 0) * baremoB;
  ...
  const colorScaleCLP = (val, metaCLP) => {
    if (val <= 0) return 'transparent';
    const pct = val / metaCLP;
    if (pct < 0.5) return 'bg-rose-900/40 text-rose-200';
    if (pct < 0.8) return 'bg-amber-900/40 text-amber-200';
    if (pct < 1.0) return 'bg-indigo-900/40 text-indigo-200';
    if (pct < 1.2) return 'bg-emerald-900/40 text-emerald-200';
    return 'bg-emerald-500 text-white font-black shadow-[0_0_15px_rgba(16,185,129,0.4)]';
  };
  ...
  const handleDownloadExcel = (type) => {
    ...
    const dayNumbersRow = [
      ...
      ...days,
      'ESTADO', isCLP ? 'TOTAL CLP' : 'TOTAL ORD', 'PROM. DIARIO', 'PROY. CIERRE'
    ];

    const sortedTechs = getSortedTechs(isCLP ? 'clp' : 'orders');
    const dataRows = sortedTechs.map(t => {
      ...
      const dailyValues = days.map(d => {
        ...
        const valPts = (dayData?.pts || 0);
        return isCLP ? (valPts * baremo) : (dayData?.orders || dayData?.count || 0);
      });
      ...
      return [
        ...
        ...dailyValues,
        t.estado || t.status || '',
        isCLP ? formatCLP(currentTotal) : currentTotal,
        isCLP ? formatCLP(avgProd) : avgProd.toFixed(1),
        isCLP ? formatCLP(proyeccion) : proyeccion.toFixed(1)
      ];
    });

    const worksheet = XLSX.utils.aoa_to_sheet([dayNamesRow, dayNumbersRow, ...dataRows]);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, isCLP ? 'Producción CLP' : 'Volumen ORD');
    XLSX.writeFile(workbook, `Reporte_Financiero_${isCLP ? 'CLP' : 'ORD'}_${year}_${month + 1}.xlsx`);
  };

  const renderTable = (type, title) => {
    const isCLP = type === 'clp';
    const mainColor = isCLP ? 'text-emerald-400' : 'text-cyan-400';
    const sortedTechs = getSortedTechs(isCLP ? 'clp' : 'orders');
      ...
      <div className="w-full mb-10">
        <div className="flex items-center justify-between px-4 py-2 bg-slate-900 border border-slate-800 rounded-t-xl">
          <div className="flex items-center gap-3">
            <div className={`w-1.5 h-5 ${isCLP ? 'bg-emerald-500' : 'bg-cyan-500'} rounded-full`} />
          ...
          <table className="w-full border-collapse p-0 m-0" style={{ borderSpacing: 0, borderCollapse: 'collapse' }}>
            <thead>
                ...
                {Array.from({length: Math.ceil((days.length + new Date(Date.UTC(year, month, 1)).getUTCDay() - 1) / 7) + 1}).map((_, idx) => {
                  ...
                  const weekTotal = sortedTechs.reduce((acc, t) => {
                    ...
                    return acc + daysInThisWeekArray.reduce((daySum, d) => {
                      const dateKey = `${year}-${String(month + 1).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
                      const valPts = (t.dailyMap?.[dateKey]?.pts || 0);
                      return daySum + (isCLP ? (valPts * baremo) : (t.dailyMap?.[dateKey]?.orders || t.dailyMap?.[dateKey]?.count || 0));
                    }, 0);
                  }, 0);

                  const productiveDaysInWeek = daysInThisWeekArray.filter(d => {
                    const dateKey = `${year}-${String(month + 1).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
                    return sortedTechs.some(t => (t.dailyMap?.[dateKey]?.pts || 0) > 0);
                  }).length;
                  ...
                  return (
                    <th key={idx} colSpan={daysInThisWeekArray.length} className={`border-r-2 border-white/20 text-[9px] font-black uppercase tracking-tight ${themeColor} bg-slate-900/90 h-7`}>
                      <div className="leading-tight mb-0.5">SEMANA {idx + 1}</div>
                      <div className="text-[8px] flex justify-center gap-2 font-black">
                        <span className="text-white">T: {isCLP ? formatCLP(weekTotal) : Math.round(weekTotal)}</span>
                        <span className="text-amber-400">P: {isCLP ? formatCLP(weekAvg) : weekAvg.toFixed(1)}</span>
                      </div>
                    </th>
                  );
                })}
              ...
              <tr className="bg-slate-900 border-b border-slate-800 sticky top-0 z-30 h-6">
                ...
                <th className="sticky right-[165px] z-30 bg-slate-900 border-l-4 border-l-white w-[35px] min-w-[35px] text-[7px] font-black text-slate-400 uppercase p-0 text-center">EST</th>
                <th className="sticky right-[110px] z-30 bg-slate-900 border-l border-slate-800 w-[55px] min-w-[55px] text-[7px] font-black text-slate-400 uppercase p-0 text-center">{isCLP ? 'TOTAL CLP' : 'TOTAL ORD'}</th>
                <th className="sticky right-[55px] z-30 bg-slate-900 border-l border-slate-800 w-[55px] min-w-[55px] text-[7px] font-black text-slate-400 uppercase p-0 text-center">PROM</th>
                 ...
                 return (
                   <tr key={t.idUnique || t._id} className="h-6 border-b border-white/5 hover:bg-white/5 p-0 m-0">
                     ...
                     {days.map(d => {
                       ...
                       const dayData = t.dailyMap?.[dateKey];
                       const valPts = (dayData?.pts || 0);
                       const val = isCLP ? (valPts * baremo) : (dayData?.orders || dayData?.count || 0);
                       const isSunday = new Date(Date.UTC(year, month, d)).getUTCDay() === 0;
                       const isHoliday = holidays.includes(dateKey);
                       let content = null, style = "";
                       if (valPts > 0) {
                         content = <div className={`w-full h-full flex items-center justify-center font-black text-[7px] ${isCLP ? colorScaleCLP(val, metaCLP) : colorScaleOrders(val)}`}>{isCLP ? formatCLP(val).replace('$','') : val}</div>;
                       } else if (isSunday) {
                     ...
                     <td className="sticky right-[165px] z-20 bg-slate-900 border-l-4 border-l-white text-center p-0 m-0"><div className="text-[6px] font-black text-slate-500">{(t.estado || t.status || '').substring(0,4)}</div></td>
                     <td className="sticky right-[110px] z-20 bg-slate-900 border-l border-slate-800 text-center text-[8px] font-black text-white p-0 m-0">{isCLP ? formatCLP(currentTotal) : currentTotal}</td>
                     <td className="sticky right-[55px] z-20 bg-slate-900 border-l border-slate-800 text-center text-[8px] font-black text-slate-400 p-0 m-0">{isCLP ? formatCLP(avgProd) : avgProd.toFixed(1)}</td>
                     <td className="sticky right-0 z-20 bg-slate-900 border-l border-slate-800 text-center text-[8px] font-black p-0 m-0 text-indigo-400">{isCLP ? formatCLP(proyeccion) : proyeccion.toFixed(1)}</td>
                   </tr>
                 );
              })}
            </tbody>
               ...
               <tr className="h-6 border-t border-white/20">
                 ...
                 {days.map(d => {
                   ...
                   const total = sortedTechs.reduce((s, t) => {
                     const baremo = getBaremo(t.proyecto || t.cliente);
                     const valPts = (t.dailyMap?.[dateKey]?.pts || 0);
                     return s + (isCLP ? (valPts * baremo) : (t.dailyMap?.[dateKey]?.orders || t.dailyMap?.[dateKey]?.count || 0));
                   }, 0);
                   const { isNewWeek } = getWeekInfo(d);
                   return <td key={d} className={`p-0 text-center text-[7px] font-black ${mainColor} border-r border-white/5 ${getWeekStyle(d)} ${isNewWeek ? 'border-l-2 border-l-white/20' : ''}`}>{total > 0 ? (isCLP ? formatCLP(total).replace('$','') : total.toFixed(1)) : '—'}</td>;
                 })}
                 <td className="sticky right-[165px] z-20 bg-slate-900 border-l-4 border-l-white p-0"></td>
                 <td className="sticky right-[110px] z-20 bg-slate-900 border-l border-slate-800 text-center text-[9px] font-black text-white p-0">
                    {(() => {
                      const total = sortedTechs.reduce((s, t) => {
                        const baremo = getBaremo(t.proyecto || t.cliente);
                        const techTotalPts = Object.values(t.dailyMap || {}).reduce((acc, d) => acc + (typeof d === 'object' ? (d.pts || 0) : d), 0);
                        return s + (isCLP ? (techTotalPts * baremo) : (t.orders || 0));
                      }, 0);
                      return isCLP ? formatCLP(total) : total.toFixed(1);
                    })()}
                 </td>
                 ...
                 <td className="sticky right-0 z-20 bg-slate-900 border-l border-slate-800 text-center text-[9px] font-black text-indigo-400 p-0">
                    {(() => {
                      const totalVal = sortedTechs.reduce((s, t) => {
                        const baremo = getBaremo(t.proyecto || t.cliente);
                        const techTotalPts = Object.values(t.dailyMap || {}).reduce((acc, d) => acc + (typeof d === 'object' ? (d.pts || 0) : d), 0);
                        return s + (isCLP ? (techTotalPts * baremo) : (t.orders || 0));
                      }, 0);
                      ...
                      const proy = avgDay * (metaConfig?.diasLaboralesMes || 22);
                      return isCLP ? formatCLP(proy) : proy.toFixed(1);
                    })()}
                 </td>
               </tr>
            </tfoot>
  ...
  return (
    <div className="flex flex-col gap-6 pb-20">
      {renderTable('clp', 'TABLA 01: PRODUCCIÓN FINANCIERA (CLP)')}
      {renderTable('orders', 'TABLA 02: VOLUMEN (ÓRDENES)')}
    </div>
  );
};

export default FinancialDiaTable;
